        ...
        if (isset($_REQUEST['intent_id']) && !empty($_REQUEST['intent_id']) && isset($_REQUEST['_wpnonce'])) { 
            if(wp_verify_nonce( $_REQUEST['_wpnonce'], 'eh_stripe_show_qr' ) ) { 
               ...
               if (isset($intent->next_action->wechat_pay_display_qr_code->image_data_url)) {
                   ...
                   ?><div id="eh-wechat-qr" <?php print($qr_style) ?> ><center><img width="<?php print($width); ?>" src="<?php print($intent->next_action->wechat_pay_display_qr_code->image_data_url) ?>" ></center>
                    <style>.payment_methods{ display: none; }</style>
                    <p style="text-align: center; font-weight: bold; font-family: Arial, sans-serif;">WeChat Pay QR</p>
                    </div> <?php
